...
*	      "credito_abono" --> Lista de los deudores de credito y cuanto han abonado y se envia la fecha de hoy               *					   el formato es AAAAMMDD
...
*	      "pedidos_paso1"  --> Imprimer los pedidos para el Carnicero (Paso 1 de pedidos).
				Se envia el numero de pedido en la primera variable.
*
*
...
int imprimirticket(int id_venta_num, char tipo[20], double pago_num){
...
/*        int numVenta; //Numero de venta*/
	...
	char id_venta[20], cambio[10], pago[10], num_articulos[300], peso[300], totales[20], total_cierre_listado[40];
...
//CONFIGURACION IMPRESORA
	...
	char impresora_credito[20];
	char impresora_contado[20];
	char impresora_factura[20] ;
	char impresora_corte_caja[20];
	char impresora_credito_abono[20];
	char impresora_corte_all_caja[20];
	char impresora_pedidos_paso1[20];
...
//	int tam_seek;
	...
	static char subraya_s[4],negrita_grande[4],negrita_subraya[4],negrita[4];
...
return (1);
		...
		if(fpt2 == NULL){
			...
			imprimir(tamano1,nX);
			imprimir(alinea_i,nX);
			//Inserta la informaciÃ?n del ticket
			if(conecta_bd() == -1){
				g_print("\nNo me puedo conectar a la base de datos\n");
				return (1);
			}else{

				sprintf(sql,"SELECT Venta.id_venta, Venta.fecha, Venta.hora, FORMAT(Venta.monto,2), LEFT(CONCAT( Usuario.nombre,  ' ', Usuario.apellido ),27)  AS cajero, LEFT(Cliente.nombre,38) AS cliente, Cliente.domicilio, Cliente.colonia, Cliente.entre_calles, Cliente.id_cliente, Cliente.ciudad_estado, FORMAT(%f - Venta.monto,2) FROM Venta, Usuario, Cliente, Empleado WHERE Usuario.id_usuario = Venta.id_usuario AND Cliente.id_cliente = Venta.id_cliente AND Venta.id_venta = %s GROUP BY Venta.id_venta", pago_num, id_venta);
				printf("%s\n", sql);
				err = mysql_query(&mysql, sql);
					...
					if(resultado){
							if (mysql_num_rows(resultado) > 0){
								...
								while((row = mysql_fetch_row(resultado))){
									...
									{
										num_articulos_int = num_articulos_int + atof(row[5]);
										strcpy(num_tmp,"  ");
									}
									//Nombre Producto
									if(strlen(row[0]) > 12)
									...
									if(strlen(row[0]) > 12)
										...
										sprintf(c, "%s%s", row[1],num_tmp);	//Cantidad
...
//		imprimir(c,nX);
		...
		system(cad_temporal);
  		//system("cat impresion/impresiones-tmp.txt > /dev/lp0");
		//system("lpr -Pminiprinter impresion/impresiones-tmp.txt");
		//system("lpr -#1 impresion/impresiones-tmp.txt");
		}
...
/*													cambio_num = cambio_num + atof(listatipos[1]);*/
	...
	/*
												//SE QUITA PORQUE NO LO QUIEREN!, PERO IGUAL LUEGO LO PIDEN! =)
												//Calcula el subtotal
												cambio_num = atof(listatipos[1]);
												sprintf(cambio, "%.2f", cambio_num);
												strcpy(temp2,"");
												strcpy(c,"");
												strcpy(temp2,"Subtotal: $ ");
												strncat(c,temp3,23-strlen(temp2));
												strcat(c, temp2);
												...
												strcat(c, "\n");
												imprimir(c,nX);
												fac_subtotal_num = fac_subtotal_num + cambio_num;

												//Calcula el IVA
												//cambio_num = atof(listatipos[1]) - cambio_num;
												cambio_num = 0;
												sprintf(cambio, "%.2f", cambio_num);
												strcpy(temp2,"");
												strcpy(c,"");
												...
												strcat(c, "\n");
												imprimir(c,nX);
												fac_iva_num = fac_iva_num + cambio_num;
												...
												strcat(c, "\n\n");
												imprimir(c,nX);
												fac_total_num = fac_total_num + cambio_num;
	*/
											...
											if(bandera>1){ //Verifica si existe Credito y contado para mostrar la suma de los 2
												...
												imprimir(c,nX);
												//Calcula el Total
												sprintf(cambio, "%.2f", fac_total_num);
												strcpy(temp2,"");
												strcpy(c,"");
												strcpy(temp2,"TOTAL: $ ");
												strncat(c,temp3,23-strlen(temp2));
									...
									if((resultado = mysql_store_result(&mysql))){
										...
										if (mysql_num_rows(resultado) > 0){
											...
											while((row = mysql_fetch_row(resultado))){
												...
												if(strcmp(row[0], "contado") == 0 ){ //Es credito o contado
											...
											if(fac_cont_total > 0){
												...
												strncat(c,temp3,23-strlen(temp2));
												strcat(c, temp2);
												strncat(c,temp3,11-strlen(listatipos[1]));
												strcat(c, listatipos[1]);
												imprimir(c,nX);
												imprimir(salto,nX);
												/*
												sprintf(c, "CONTADO:\n"); imprimir(c,nX);
											...
											if(bandera == 2){ //Verifica si existe Credito y contado para mostrar la suma de los 2
												...
												/*
												fac_credcont_subtotal = fac_cred_subtotal + fac_cont_subtotal;
												fac_credcont_iva = fac_cred_iva + fac_cont_iva;
												fac_credcont_total = fac_cred_total + fac_cont_total;
												fac_credcont_subtotal = fac_credcont_total; //Se pone ASI porque el IVA es 0
												strcpy(temp2,"");
												strcpy(c,""); strcpy(cambio,"---------");
												strcpy(temp2," ");
												strncat(c,temp3,23-strlen(temp2));
												...
												imprimir(c,nX);
												//Calcula el Total
												sprintf(cambio, "%.2f", fac_credcont_subtotal);
												strcpy(temp2,"");
												strcpy(c,"");
												strcpy(temp2,"Subtotal: $ ");
												strncat(c,temp3,23-strlen(temp2));
												...
												imprimir(c,nX);
												//Calcula el Total
												sprintf(cambio, "%.2f", fac_credcont_iva);
												strcpy(temp2,"");
												strcpy(c,"");
												strcpy(temp2,"IVA: $ ");
												strncat(c,temp3,23-strlen(temp2));
												...
												imprimir(c,nX);
												//Calcula el Total
												sprintf(cambio, "%.2f", fac_credcont_total);
												strcpy(temp2,"");
												strcpy(c,"");
												strcpy(temp2,"TOTAL: $ ");
												strncat(c,temp3,23-strlen(temp2));
							...
							strcpy(sql,"SELECT Venta_Articulo.id_articulo, /*FORMAT(*/SUM( Venta_Articulo.monto )/*,2)*/  AS monto_IVA, /*FORMAT(*/(SUM( Venta_Articulo.monto )  / 1.15)/*,2)*/ AS monto, SUM( Venta_Articulo.cantidad )  AS cantidad, Articulo.nombre FROM Corte_Caja, Venta, Venta_Articulo, Articulo WHERE Corte_Caja.id_caja = Venta.id_caja AND Venta.id_venta BETWEEN Corte_Caja.id_venta_inicio AND Corte_Caja.id_venta_fin AND Venta_Articulo.id_venta = Venta.id_venta AND Articulo.id_articulo = Venta_Articulo.id_articulo AND Venta.cancelada =  'n' AND Corte_Caja.id_corte_caja = ");
							strcat(sql, id_venta);
							strcat(sql," GROUP  BY Venta_Articulo.id_articulo ORDER BY cantidad");
							...
							system(cad_temporal);
							//printf("\n%s",cad_temporal);
							//system("cat impresion/impresiones-tmp.txt > /dev/ttyS1");
							//system("lpr -Pminiprinter impresion/impresiones-tmp.txt");
...
//		imprimir(c,nX);
		...
		fpt = fopen(TicketImpresion,"w");
		if(fpt == NULL){
			printf("\nERR	OR no se puede abrir el archivo a imprimir");
			return (1);
		}
		else
		{
		...
		}else
		{
			...
			sprintf(sql, "SELECT Pedido.fecha, Pedido.hora, Pedido.HoraEntrega, Pedido.FechaEntrega FROM Pedido INNER JOIN Cliente ON Cliente.id_cliente = Pedido.id_cliente WHERE Pedido.id_pedido=%s", id_venta);
