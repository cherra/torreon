...
//	int tam_seek;
	...
	gchar *listatipos[13]; //Se guarda la consulta SQL
//MINIPRINTER EPSON
	static char salto[4];
				...
				if((resultado = mysql_store_result(&mysql))){
					if((row = mysql_fetch_row(resultado))){
						if (mysql_num_rows(resultado) > 0){
							...
							if (corte_final)
							{
								sprintf(sql,"SELECT FORMAT(SUM(Venta_Articulo.monto),2), FORMAT(SUM(Venta_Articulo.cantidad),3), CONCAT(Empleado.nombre,' ',Empleado.apellido) AS Vendedor, ' ' AS Bascula, Empleado.id_empleado FROM Corte_Caja INNER  JOIN Venta ON Corte_Caja.id_caja = Venta.id_caja INNER JOIN Venta_Articulo ON Venta_Articulo.id_venta = Venta.id_venta INNER JOIN Empleado ON Empleado.id_empleado = Venta.id_empleado WHERE Venta_Articulo.id_venta BETWEEN Corte_Caja.id_venta_inicio AND Corte_Caja.id_venta_fin AND Venta.cancelada = 'n' AND Corte_Caja.id_corte_caja = %s  GROUP BY Venta.id_empleado ORDER BY Vendedor", id_venta);
...
//							strcpy(sql,"SELECT Venta.id_cliente FROM Corte_Caja, Venta WHERE Corte_Caja.id_caja = Venta.id_caja AND Venta.id_venta BETWEEN Corte_Caja.id_venta_inicio AND Corte_Caja.id_venta_fin AND Venta.cancelada =  'n' AND Corte_Caja.fecha =  '");
	...
	//printf ("\nLONG: %d",strlen(listatipos[3]));
							...
							//if (corte_final)
							{
								sprintf(sql,"SELECT FORMAT(SUM(Venta_Articulo.monto),2), FORMAT(SUM(Venta_Articulo.cantidad),3), CONCAT(Empleado.nombre,' ',Empleado.apellido) AS Vendedor, ' ' AS Bascula, Empleado.id_empleado FROM Corte_Caja INNER  JOIN Venta ON Corte_Caja.id_caja = Venta.id_caja INNER JOIN Venta_Articulo ON Venta_Articulo.id_venta = Venta.id_venta INNER JOIN Articulo_Lista ON Articulo_Lista.id_articulo = Venta_Articulo.id_articulo INNER JOIN Empleado ON Empleado.id_empleado = Venta.id_empleado WHERE Venta_Articulo.id_venta BETWEEN Corte_Caja.id_venta_inicio AND Corte_Caja.id_venta_fin AND Venta.cancelada = 'n' AND Corte_Caja.fecha = '%s' AND Articulo_Lista.id_lista = 1 GROUP BY Venta.id_empleado ORDER BY Vendedor ", id_venta);
